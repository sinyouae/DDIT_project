		...
		<div class="h-100">
            ...
            <c:choose>
            	...
            	<c:otherwise>
            		...
            		<div class="todoList${todo.tdNo } d-flex flex-row" id="todoList">
            			
         
            			<c:choose>
            				<c:when test="${todo.finYn eq 'Y' }">
            					<input type="checkbox"  class="finYnCheck form-check-input" id="finYn${todo.tdNo }" value="${todo.finYn }" onclick=changYn(${todo.tdNo }) checked="checked">  
            					<input type="text" class="tdContent${todo.tdNo } no-border-input" id="tdContent${todo.tdNo } " name="tdContent" value="${todo.tdContent }" style="text-decoration: line-through; background-color: transparent; border: none; " disabled="true">
            				</c:when>
            				<c:otherwise>
            					<input type="checkbox"  class="finYnCheck form-check-input" id="finYn${todo.tdNo }" value="${todo.finYn }" onclick=changYn(${todo.tdNo })>
            					<input type="text" class="tdContent${todo.tdNo } no-border-input " id="tdContent${todo.tdNo }" name="tdContent" value="${todo.tdContent }" style="border: none; background-color: transparent; "disabled="true">
            				</c:otherwise>
            			</c:choose>
   			   			<c:choose>
			...
			<div class="btn btn-sm btn-link d-block py-2 d-flex flex-row"  onclick=insertForm()>
				 ...
				 $.ajax({
		    		...
		    		success : function(response){
						if(response.status === "success"){
							console.log("r")
							if(finYN.val() === 'Y'){
								finYN.siblings('input[id^=tdContent]').css("text-decoration", "line-through");
							}else {
								finYN.siblings('input[id^=tdContent]').css("text-decoration", "");
							}
						} else {
							console.log("s")
						}
				...
				success : function(res){
			    	...
			    	txt += `
            			...
            			if(res.todo.importYn === 'Y'){
            				...
            				if(res.todo.finYn === 'Y'){
            					txt += `
            					<input type="text" class="tdContent\${res.todo.tdNo } no-border-input" id="tdContent${todo.tdNo } " name="tdContent" value="${todo.tdContent }" style="text-decoration: line-through; background-color: transparent; border: none; " disabled="true">
            					`;
            				} else {
            					txt += `
	            					<input type="text" class="tdContent\${res.todo.tdNo } no-border-input" id="tdContent\${res.todo.tdNo }" name="tdContent" value="\${res.todo.tdContent }" style="border: none; background-color: transparent; "disabled="true">
            					`;
            				}
            			txt += `
